#!/usr/bin/env npx tsx
/**
 * Test runner for the design capability.
 *
 * Runs all fixtures in fixtures/design/ through the engine,
 * records pass/fail, and summarizes validation failure categories.
 *
 * Usage:
 *   TPDC_ADAPTER=mock npx tsx scripts/test-design.ts       # mock (no LLM)
 *   npx tsx scripts/test-design.ts                          # Claude Code CLI (Max tokens)
 *   TPDC_ADAPTER=api ANTHROPIC_API_KEY=sk-... npx tsx ...   # API direct
 *   TPDC_MODEL=opus npx tsx scripts/test-design.ts          # override model
 */
...
const FIXTURES_DIR = path.resolve(__dirname, "../fixtures/design");
...
async function main() {
  ...
  console.log(`\n========================================`);
  console.log(`  TPDC Engine — Design Test Suite`);
  ...
  for (const file of fixtureFiles) {
    ...
    try {
      const result: RunResult = await runCapability("design", input, { llm });
      console.log = origLog;
  ...
  const reportPath = path.join(reportDir, `test-design-${Date.now()}.json`);
  const report = {
