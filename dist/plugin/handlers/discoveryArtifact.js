...
exports.buildDiscoveryArtifact = buildDiscoveryArtifact;
 ...
 */
function buildDiscoveryArtifact(run, rawIdea, likelyCommand) {
    ...
    const problemStatement = intake?.problem_statement || "";
    const designContext = design?.context;
    ...
    const nfc = intake?.non_functional_constraints || [];
    ...
    // Risks — from design
    const risks = (design?.risks || []).map((r) => ({
        risk: r.risk,
        trigger: r.trigger,
    ...
    const recommendation = design?.decision || "";
    ...
    const { readiness, readinessReason } = determineReadiness(decompose, criticalQuestions, informationalQuestions, run);
    // Suggested next command — natural, specific, copy-paste ready
    const suggestedNextCommand = buildNextCommand(readiness, likelyCommand, title, rawIdea, recommendation, problemFraming, affectedAreas, risks);
    return {
        title,
        idea: rawIdea,
        problemFraming,
...
// ── Internal helpers ─────────────────────────────────────────────────
function mergeQuestions(intake, design, decompose) {
    const seen = new Set();
    const result = [];
...
function buildTradeoffs(alternatives, design) {
    const tradeoffs = [];
 ...
 */
function extractDecisionDrivers(recommendation, design, problemFraming) {
    const drivers = [];
 ...
 */
function extractImpactAreas(affectedAreas, design, intake) {
    const impact = [];
    const seen = new Set();
...
function determineReadiness(decompose, criticalQuestions, informationalQuestions, run) {
...
function buildNextCommand(readiness, likelyCommand, title, rawIdea, recommendation, problemFraming, affectedAreas, risks) {
...
function hasTopicOverlap(a, b) {
    ...
    const wordsB = new Set(b.toLowerCase().split(/\W+/).filter((w) => w.length > 3 && !stopWords.has(w)));
    let overlap = 0;
...
function synthesizeContext(title, problemFraming, affectedAreas) {
...
function synthesizePurpose(problemFraming, risks) {
...
function containsContext(action, affectedAreas) {
...
//# sourceMappingURL=discoveryArtifact.js.map