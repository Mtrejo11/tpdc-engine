...
exports.renderBugResult = renderBugResult;
const bugNormalizer_1 = require("../handlers/bugNormalizer");
const local_1 = require("../../storage/local");
function renderBugResult(run, bugCtx) {
    const lines = [];
    ...
    if (ext.platform || ext.screen) {
        ...
        lines.push(`  ${"─".repeat(50)}`);
        if (ext.platform)
            lines.push(`  Platform:  ${ext.platform}`);
        if (ext.screen)
            lines.push(`  Screen:    ${ext.screen}`);
        if (ext.actualBehavior && ext.actualBehavior !== bugCtx.rawInput)
            lines.push(`  Actual:    ${truncate(ext.actualBehavior, 55)}`);
        if (ext.expectedBehavior)
            lines.push(`  Expected:  ${truncate(ext.expectedBehavior, 55)}`);
        if (ext.reproContext)
            lines.push(`  Repro:     ${truncate(ext.reproContext, 55)}`);
        lines.push("");
    }
    // Missing context
    ...
    if (bugCtx.missingFields.length > 0) {
        ...
        lines.push(`  ${"─".repeat(50)}`);
        const suggestion = (0, bugNormalizer_1.suggestClarifiedReport)(bugCtx);
    ...
    if (execute?.touchedArtifacts) {
        const artifacts = execute.touchedArtifacts;
...
//# sourceMappingURL=bugRenderer.js.map