...
export function renderPlanResult(run: RunSummary, artifact: PlanSummaryArtifact): string {
  const lines: string[] = [];
...
export function renderPlanMarkdown(
  ...
  if (artifact.scope.length > 0) {
    lines.push("## Scope");
    lines.push("");
    for (const s of artifact.scope) lines.push(`- ${s}`);
    lines.push("");
  }
  ...
  if (artifact.likelyFiles.length > 0) {
    ...
    lines.push("");
    for (const f of artifact.likelyFiles) lines.push(`- \`${f}\``);
    lines.push("");
  }
  ...
  if (artifact.dependencies.length > 0) {
    lines.push("## Dependencies");
    lines.push("");
    for (const d of artifact.dependencies) lines.push(`- ${d}`);
    lines.push("");
  }
  ...
  if (artifact.validationApproach.length > 0) {
    lines.push("## Validation Approach");
    lines.push("");
    for (const v of artifact.validationApproach) lines.push(`- [ ] ${v}`);
    lines.push("");
  }
