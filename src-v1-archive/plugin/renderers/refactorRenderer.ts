...
export function renderRefactorResult(run: RunSummary, artifact: RefactorArtifact): string {
  const lines: string[] = [];
  ...
  if (run.mutation) {
    ...
    if (run.mutation.branchName) lines.push(`  Branch:  ${run.mutation.branchName}`);
    if (run.mutation.commitHash) lines.push(`  Commit:  ${run.mutation.commitHash.substring(0, 12)}`);
