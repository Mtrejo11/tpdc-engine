...
export function extractLearnings(
  ...
  const suggestedHeuristics: string[] = [];
  const tags: string[] = [command, run.executionMode];
  ...
  if (decompose?.status === "blocked") {
    ...
    if (questions) {
      for (const q of questions) {
        if (/platform|ios|android/i.test(q.question)) failurePatterns.push("missing_platform");
        if (/which\s+(screen|component|module|service)/i.test(q.question)) failurePatterns.push("missing_component");
        if (/what\s+(database|storage|backend)/i.test(q.question)) failurePatterns.push("missing_infrastructure");
      }
    }
  }
  ...
  if (dryRun) {
    const safe = dryRun.safe as boolean | undefined;
    const applicable = dryRun.applicable as number | undefined;
    const conflicts = dryRun.conflicts as number | undefined;
  ...
  if (run.finalVerdict === "pass") {
    ...
    if (intake) {
      const hasAC = (intake.acceptance_criteria as string[] | undefined)?.length;
      const hasAssumptions = (intake.assumptions as string[] | undefined)?.length;
      if (hasAC && hasAC > 0) successPatterns.push("clear_acceptance_criteria");
      if (hasAssumptions && hasAssumptions > 0) successPatterns.push("explicit_assumptions");
    }
  }
