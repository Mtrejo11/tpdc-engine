#!/usr/bin/env npx tsx
/**
 * Test runner for the validate capability.
 *
 * Runs all fixtures in fixtures/validate/ through the engine,
 * records pass/fail, and reports verdict + score per fixture.
 */
...
async function main() {
  ...
  console.log(`\n========================================`);
  console.log(`  TPDC Engine — Validate Test Suite`);
  ...
  for (const file of fixtureFiles) {
    ...
    try {
      const result: RunResult = await runCapability("validate", input, { llm });
      console.log = origLog;
      ...
      if (result.validated) {
        console.log(`PASS  (${result.metadata.durationMs}ms)  ${verdict.toUpperCase()} score=${score}  findings=${findings?.length ?? 0}`);
        results.push({
          fixture: label,
          pass: true,
          runId: result.runId,
  ...
  const reportPath = path.join(reportDir, `test-validate-${Date.now()}.json`);
