...
import { queryLessons } from "./store";
import { AggregatedLesson } from "./types";
...
export function injectLessons(
  ...
  const lessons = queryLessons(command, tags);

  if (lessons.length === 0) return request;
  ...
  if (hints.length === 0) return request;
...
function formatBlockLesson(lesson: AggregatedLesson): string {
